...
class ApkInstallResultReceiver : BroadcastReceiver() {
  override fun onReceive(context: Context, intent: Intent) {
    if (intent.action != ApkInstallBridge.ACTION_INSTALL_STATUS) return
    ...
    if (status == PackageInstaller.STATUS_SUCCESS || status == PackageInstaller.STATUS_FAILURE_ABORTED) {
      ApkInstallBridge.resolve(ApkInstallBridge.statusName(status))
      return
    }

    ApkInstallBridge.reject(
      "ERR_INSTALL_FAILED",
      message ?: "The system installer could not complete the update."
    )
  }
