...
| 2026-09-07 | Spendly data access (personal tree, shared collections, public links) | [SPENDLY_DATA_ACCESS_AUDIT_2026-09-07.md](SPENDLY_DATA_ACCESS_AUDIT_2026-09-07.md) | 1×P0, 3×P1 found. P0 (every personal query denied by an unbound recursive wildcard) and P1-1 (dead pre-sign-in update prompt) fixed; 2×P1 anonymous enumeration of public links left open pending a slug-as-doc-id migration. 34 new rules tests; rules coverage extended from Ganesh-only to the Spendly surface. |
| 2026-09-12 | Spendly forensic Firebase audit (security rules, auth, realtime listeners, Firestore cost, financial integrity, races, offline/sync, EPF, portfolio, architecture, tests) — diagnostic only | [SPENDLY_FIREBASE_AUDIT_2026-09-12.md](SPENDLY_FIREBASE_AUDIT_2026-09-12.md) | 42 findings (1×CRITICAL, 16×HIGH, 18×MEDIUM, 7×LOW), each filed as KAN-79…KAN-120. 21-case emulator probe proved every rules finding. Headline: `system_settings/global` writable by any user; `role` self-escalation; vault/split mutation rights UI-only; native offline persistence is memory-only; 35 dashboard listeners with unbounded history; non-atomic/non-idempotent financial writes; EPF read path drops lifecycle fields. Nothing fixed in this pass. |
